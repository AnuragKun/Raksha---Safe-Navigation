...
class AuthRepositoryImpl(
    ...
    override suspend fun verifyOtp(verificationId: String, code: String): Result<String> {
        return try {
            ...
            if (user != null) {
                // Try linking first; if phone is already linked, fall back to updatePhoneNumber
                try {
                    user.linkWithCredential(credential).await()
                } catch (e: FirebaseAuthUserCollisionException) {
                    Log.d("AuthRepositoryImpl", "Phone already linked, updating phone number instead")
                    user.updatePhoneNumber(credential).await()
                }

                // Reload user to get updated phone number
                user.reload().await()
